...
public interface IMain extends ICommon{
    ...
    + gs(4) + "Games::comment" + gs(14) + "游戏配置集节点结构说明，对执行游戏无影响，仅此说明而已。" + gl(1)
    + gs(4) + "Games::Game" + gs(17) + "游戏配置节点，包括执行游戏的命令行参数配置及执行游戏前和执行游戏后的BAT脚本命令配置等等。" + gl(1)
    + gs(4) + "Game节点由comment、name、id、path、exe、args、priority、icon、agentExecutablePath、agentArgs、before、after、beforeWait、afterWait、watchWait、watch节点按顺序组成；comment节点必须在最前面，watch节点可以有多个。" + gl(1)
    + gs(4) + "Game::comment" + gs(15) + "游戏快捷方式说明，默认值同Game::name。" + gl(1)
    + gs(4) + "Game::name" + gs(18) + "游戏快捷方式名称，一般使用游戏中文名称。" + gl(1)
    + gs(4) + "Game::id" + gs(20) + "游戏唯一标识。" + gl(1)
    + gs(4) + "Game::path" + gs(18) + "游戏可执行文件路径，也是Game::icon的路径。" + gl(1)
    + gs(4) + "Game::exe" + gs(19) + "游戏可执行文件名称，不包含文件扩展名" + FILE_SUFFIX_EXE + "；若Game::agentExecutablePath非空且有效，则优先使用代理启动游戏。" + gl(1)
    + gs(4) + "Game::args" + gs(18) + "Game::exe的命令行参数。" + gl(1)
    + gs(4) + "Game::priority" + gs(14) + "游戏进程的优先级，默认值为128；可选值为：32（标准），64（低），128（高），256（实时），16384（低于标准），32768（高于标准）。" + gl(1)
    + gs(4) + "Game::icon" + gs(18) + "游戏快捷方式的图标文件完整名称（包含文件扩展名）；若为空则使用游戏可执行文件中图标。" + gl(1)
    + gs(4) + "Game::agentExecutablePath" + gs(3) + "Game::exe的代理可执行文件绝对路径名；适用于使用游戏插件启动游戏的情况，例如：上古卷轴5的skse。" + gl(1)
    + gs(4) + "Game::agentArgs" + gs(13) + "Game::agentExecutablePath的命令行参数；若想使用代理参数指定游戏进程优先级，则Game::priority应该置空，否则优先使用Game::priority。" + gl(1)
    + gs(4) + "Game::before" + gs(16) + "在游戏执行前需要执行的BAT脚本命令。" + gl(1)
    + gs(4) + "Game::after" + gs(17) + "在游戏执行后需要执行的BAT脚本命令。" + gl(1)
    + gs(4) + "Game::beforeWait" + gs(12) + "Game::before命令执行完后等待beforeWait秒，再执行游戏；仅当Game::before不为空时生效，默认值为10；基于性能考虑，取值范围为：1~60，若超过取值范围程序会取默认值。" + gl(1)
    + gs(4) + "Game::afterWait" + gs(13) + "执行游戏后等待afterWait秒，再执行Game::after命令；仅当Game::after不为空时生效，默认值为10；基于性能考虑，取值范围为：1~60，若超过取值范围程序会取默认值。" + gl(1)
    + gs(4) + "Game::watchWait" + gs(13) + "游戏监控进程的等待时间，每隔watchWait秒后检测一次游戏进程是否存在；仅当Game::watch不为空时生效，默认值为10；基于性能考虑，取值范围为：1~60，若超过取值范围程序会取默认值。" + gl(1)
    + gs(4) + "Game::watch" + gs(17) + "由Game::before或Game::after脚本启动的进程的名称（例如：editplus.exe）或进程的可执行文件路径名（例如：F:/tools/EditPlus/editplus.exe），在游戏进程结束后监控程序会自动关闭之。" + gl(1) + gs(4);
