...
public interface IILCode extends IFileUtil{
    ...
    String MODE_REPL = "1";
    String MODE_ADD_AFTER = "2";
    String MODE_ADD_BEFORE = "3";
    ...
    + gs(4) + "ILCode::processingMode" + gs(6) + "在自动生成已修改的IL源文件时对IL代码的处理模式，取值范围为：" + MODE_NATIVE + "," + MODE_REPL + "," + MODE_ADD_AFTER + "," + MODE_ADD_BEFORE + "，默认值为" + MODE_NATIVE + "；取" + MODE_NATIVE + "表示提取IL源文件的原始数据；取" + MODE_REPL + "表示提取ILCode::codeFragment作为替换数据，此时ILCode::queryRegex节点和ILCode::codeRegex节点都不能为空，且ILCode::codeRegex节点值最多只能指定2个正则查询表达式，分别匹配起始代码行和终止代码行，只指定1个则表示只唯一匹配一行待替换代码；取" + MODE_ADD_AFTER + "表示提取ILCode::codeFragment作为新增数据插入到ILCode::queryRegex节点值匹配的数据块之后，此时ILCode::queryRegex节点不能为空，且ILCode::codeRegex节点值将被忽略；取" + MODE_ADD_BEFORE + "表示提取ILCode::codeFragment作为新增数据插入到ILCode::queryRegex节点值匹配的数据块之前，此时ILCode::queryRegex节点不能为空，且ILCode::codeRegex节点值将被忽略。\n"
