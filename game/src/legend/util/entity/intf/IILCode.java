...
public interface IILCode extends IFileUtil{
    ...
    + gs(4) + "ILCodes节点由节点comment、mode、header、partition和多个ILCode节点按顺序组成，comment节点必须在最前面。\n"
    ...
    + gs(4) + "ILCodes::header" + gs(13) + "IL源文件内容的头部信息行数，表示查询时忽略掉前header行；取值范围为：0~" + Integer.MAX_VALUE + "，不指定或超过取值范围则取默认值" + SIZE_IL_HEADER + "。\n"
    + gs(4) + "ILCodes::partition" + gs(10) + "IL源文件内容的分区行数，表示查询时按照每个分区partition行进行并行查询；取值范围为：" + SIZE_IL_PARTITION_MIN + "~" + Integer.MAX_VALUE + "，不指定或超过取值范围则取默认值" + SIZE_IL_PARTITION + "。\n"
