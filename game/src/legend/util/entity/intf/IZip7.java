...
public interface IZip7 extends IFileUtil{
    ...
    String ZIP7_ARG_SPF = "-spf";
    String ZIP7_ARG_SPF2 = "-spf2";
    ...
    + gs(4) + "Zip7Task::zipMode" + gs(11) + "压缩文件的输入文件路径处理模式，取值范围为：0,1,2；取0表示保存输入文件的相对路径；取1表示保存输入文件的绝对路径（包含磁盘盘符）；取2表示保存输入文件的绝对路径（不包含磁盘盘符）；若指定的值超过取值范围程序会取默认值0；仅当Zip7Task::mode值为0时有效。\n"
