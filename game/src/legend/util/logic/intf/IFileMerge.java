...
public interface IFileMerge extends IFileUtil{
    String FILE_CONFIG = "./file-merge.xml";
    String N_MERGE_CONFIG = "文件整合配置文件";
    String N_INVALIDATE = "path或path2或path3或mergeExecutablePath或queryRegex节点为空！";
    String ERR_CONFIG_NON = N_MERGE_CONFIG + S_DQM_L + FILE_CONFIG + S_DQM_R + V_NON_EXISTS;
    ...
    + gs(4) + "FileMerge::path" + gs(18) + "第一个输入文件源的绝对路径，一般设置为最新版本修改文件的目录，同时也是保存最终整合文件的目录。\n"
    + gs(4) + "FileMerge::path2" + gs(18) + "第二个输入文件源的绝对路径，一般设置为最新版本原始文件的目录。\n"
    + gs(4) + "FileMerge::path3" + gs(17) + "第三个输入文件源的绝对路径，一般设置为上一版本原始文件的目录。\n"
    ...
    + gs(4) + "Merge节点path、md5节点按顺序组成。\n"
    + gs(4) + "Merge::path" + gs(25) + "整合文件的相对路径名。\n"
    + gs(4) + "Merge::md5" + gs(26) + "文件内容对应的MD5码，由程序根据FileMerge::queryRegex查询获得的FileMerge::path2、FileMerge::path3目录中同名文件的文件内容自动生成；每次执行命令时程序会检查这两个同名文件的文件内容被修改，若已修改则会更新MD5码并进行文件整合，否则跳过该文件的整合操作。\n"+ gs(4);
}
