...
public interface IFileUtil extends ICommon{
    ...
    + "dest\t\t\t输出文件目录。" + gl(2)
    + "backup\t\t\t备份文件目录。" + gl(2)
    + "sizeExpr\t\t文件大小表达式，匹配的正则表达式为：" + REG_FLE_SIZ + "；取值范围为：0~9223372036854775807B，不指定则取默认值0B；例如：100B（不小于100字节），10KB（不小于10千字节），1-100MB（介于1兆字节到100兆字节之间），500MB;1GB（介于500兆字节到1千兆字节之间），2,1GB（介于2千兆字节到1千兆字节之间），800,800（等于800字节）。" + gl(2)
    + "replacement\t\t字符串替换表达式，可作为文件名替换正则表达式，也可作为二维表格式文件中的列字符串替换表达式；" + gl(2)
    + "split\t\t二维表格式文件中的列分隔符正则表达式，例如：[,;| \t]+；不指定则取默认值[ \t]+，即只使用空格或制表符作为列分隔符。" + gl(2)
    ...
    + "! 可添加在命令选项末尾，表示不匹配查询的根目录，如：file -fd! . d:/games 不匹配games目录，只匹配该目录中的任意文件和子目录名称。" + gl(2)
    ...
    + CMD + CMD_REP_FILE + OPTIONS + " regex src [level]" + gl(1) + "根据regex查找src中的文件（同-f），。" + gl(2)
    ...
    + CMD + CMD_FND_DIR_DIR_SIZ_ASC + "+ \\Ajp$ \"F:/games/DARK SOULS REMASTERED\" 100KB;10MB" + gl(1) + "先查询该目录中的文件和第一级目录，再按文件或目录大小递增排序。" + gl(2)
    + CMD + CMD_FND_DIR_DIR_SIZ_DSC + "+ \\Ajp$ \"F:/games/DARK SOULS REMASTERED\" 100KB;10MB" + gl(1) + "先查询该目录中的文件和第一级目录，再按文件或目录大小递减排序。" + gl(2)
    + CMD + CMD_FND_DIR_OLY_SIZ_ASC + "+ \\Ajp$ \"F:/games/DARK SOULS REMASTERED\" 100KB;10MB" + gl(1) + "先查询该目录中的第一级目录，再按目录大小递增排序。" + gl(2)
    + CMD + CMD_FND_DIR_OLY_SIZ_DSC + "+ \\Ajp$ \"F:/games/DARK SOULS REMASTERED\" 100KB;10MB" + gl(1) + "先查询该目录中的第一级目录，再按目录大小递减排序。" + gl(2)
