...
public interface IFileUtil extends ICommon{
    ...
    + RULE_REGENROW + "(rstring)" + gs(10) + "根据string重新生成每一行数据，匹配的正则表达式为：" + REG_RULE_REGENROW + "；此规则只能作为最后一条原子规则使用，即只能放在规则列表的最后面；rstring为行数据正则替换表达式，可使用特殊字符占位符表达式（见regex参数）和列数据占位符表达式；" + gl(2)
