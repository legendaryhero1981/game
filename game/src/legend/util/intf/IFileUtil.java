...
public interface IFileUtil extends IReplaceRule{
    ...
    + RULE_SINGLEROW + "(replacement)" + gs(39) + "根据replacement重新生成每一行数据，匹配的正则表达式为：" + REG_RULE_REGENROW + "；此规则为终止原子规则使用，即只能放在规则列表的最后面；replacement为行数据正则替换表达式，可使用特殊字符占位符表达式（见regex参数）和列数据占位符表达式。" + gl(2)
    + RULE_FINALSINGLEROW + "(replacement[" + SPRT_ARGS + "join" + SPRT_ARGS + "prefix" + SPRT_ARGS + "suffix])" + gs(9) + "先根据replacement重新生成每一行数据（同规则" + RULE_SINGLEROW + "），再使用jion把所有行数据连接成一行，最后在这行数据的首尾分别加上prefix、suffix；replacement必须指定，jion、prefix、suffix可以不指定，若不指定jion则使用空字符串连接每一行数据；匹配的正则表达式为：" + REG_RULE_GENFINALROW + "；此规则为终止原子规则使用，即只能放在规则列表的最后面；replacement为行数据正则替换表达式，可使用特殊字符占位符表达式（见regex参数）和列数据占位符表达式；jion为行数据连接字符串，prefix为前缀字符串，suffix为后缀字符串，均可使用特殊字符占位符表达式（见regex参数）。" + gl(2)
