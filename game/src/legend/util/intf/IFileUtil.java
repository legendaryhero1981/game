...
public interface IFileUtil extends IReplaceRule{
    ...
    + "命令选项：" + gl(2)
    + OPT_INSIDE + " 可添加在命令选项末尾，表示显示最简明的信息，也不会显示进度条；优先级高于+；若未指定则默认显示进度条；可与!或@或?连用；例如：-fd~!@?。" + gl(2)
    + OPT_DETAIL + " 可添加在命令选项末尾，表示输出详细信息；优先级低于~和*；可与!或@或?连用；例如：-fd!+@?。" + gl(2)
    + OPT_SIMULATE + " 可添加在命令选项末尾，表示模拟执行命令，不进行实际操作，仅输出详细信息；优先级高于+和~，且与~联用时只输出最简明的信息；可与!或@或?连用；例如：-fd*?@!。" + gl(2)
