...
public interface IFileUtil extends IReplaceRule{
    ...
    + "split" + gs(10) + "分隔符字符串，可作为二维表格式文件中的列分隔符正则表达式，例如：[,;| \\t]+；不指定则取默认值[ \\t]+，即只使用空格或制表符作为列分隔符；也可作为文件或目录名称中分隔名称和版本号的分隔符，不指定则取默认值空字符串（版本号从匹配的第一个数字字符串+英文点号字符开始，版本号左边的字符串则为名称）；可使用特殊字符占位符表达式（见regex参数）。" + gl(2)
    ...
    + "命令选项：" + gl(2)
    + OPT_INSIDE + " 可添加在命令选项末尾，表示显示最简明的信息，也不会显示进度条；优先级高于+和*；若未指定则默认显示进度条；可与!或@或?连用；例如：-fd~!@?。" + gl(2)
    + OPT_DETAIL + " 可添加在命令选项末尾，表示输出详细信息；优先级高于*；可与!或@或?连用；例如：-fd!+@?。" + gl(2)
    + OPT_SIMULATE + " 可添加在命令选项末尾，表示模拟执行命令，不进行实际操作，仅输出详细信息；优先级低于+和~；可与!或@或?连用；例如：-fd*?@!。" + gl(2)
    ...
    + CMD + CMD_DEL_OLD_VER + OPTIONS + "regex src [split] [level]" + gl(1) + "根据regex和split删除src中所有匹配文件的旧版本。" + gl(2)
    + CMD + CMD_DEL_DIR_OLD_VER + OPTIONS + "regex src [split] [level]" + gl(1) + "根据regex和split删除src中所有匹配文件和目录的旧版本。" + gl(2)
    + CMD + CMD_DEL_DIR_OLY_OLD_VER + OPTIONS + "regex src [split] [level]" + gl(1) + "根据regex和split删除src中所有匹配目录的旧版本。" + gl(2)
    ...
    + CMD + CMD_UPGRADE + OPTIONS + "regex src dest [backup] [level]" + gl(1) + "根据regex将src中所有匹配文件更新到dest中；若指定了backup，则更新时会先检查dest中是否已存在该文件，若存在则先将该文件备份到backup中，再更新之。" + gl(2)
    + CMD + CMD_UGD_DIR + OPTIONS + "regex src dest [backup] [level]" + gl(1) + "根据regex将src中所有匹配文件和目录及其中所有文件更新到dest中；若指定了backup，则更新时会先检查dest中是否已存在该文件，若存在则先将该文件备份到backup中，再更新之。" + gl(2)
    ...
    + CMD + CMD_REP_FLE_BT + "* (?i)\\A`temp1.txt`$ E:/Decompile/DLL-ildasm \"1@@LOWER;;UPPER=>REPLACE(\\.,,_);;SINGLE-ROW(String INST_#1-1# = #DQM##1.1##DQM#;)\" \"\\t+\" 1" + gl(1)
    ...
    + CMD + CMD_REP_FLE_BT + "* (?i)\\A`temp1.txt`$ E:/Decompile/DLL-ildasm \"1@@UPPER=>REPLACE(\\.,,_);;SINGLE-ROW(addInstruction(INST_#1-1#,#DQM##2.0##DQM#,#DQM=2#);)\" \"\\t+\" 1" + gl(1)
    + "先查询再对该目录中名称（忽略大小写）为temp1.txt的文件数据执行一系列有序的替换规则：" + gl(1)
    ...
    + CMD + CMD_REP_FLE_BT + "* (?i)`native.log`$ d:/games  \"REPLACE(.*?--initialize-at-run-time=(.+?) .*,,$1,,#EMPTY#);;FINAL-SINGLE-ROW(#1.1#,,`,`,,--initialize-at-run-time=)\" \"\\n\" 1" + gl(1)
    ...
    + CMD + CMD_REP_FLE_BT + "* (?i)`native.log`$ d:/games  \"REPLACE(.*?--initialize-at-run-time=(.+?) .*,,$1,,#EMPTY#);;DIST-FINAL-SINGLE-ROW(#1.1#,,`,`,,--initialize-at-run-time=)\" \"\\n\" 1" + gl(1)
    + "先查询再对该目录中名称（忽略大小写）为native.log的文件数据执行一系列有序的替换规则：" + gl(1)
    + "1、对每行的第1列数据执行原子规则REPLACE：将所有匹配的列字符串替换为捕获组1，且将所有不匹配的列字符串替换为空字符串；" + gl(1)
    ...
    + CMD + CMD_DEL_OLD_VER + "!* . \"E:/Java/eclipse/SpringToolSuite/plugins\" _" + gl(1) + "先查询再删除该目录中所有匹配文件的旧版本。" + gl(2)
    + CMD + CMD_DEL_DIR_OLD_VER + "!* . \"E:/Java/eclipse/SpringToolSuite/plugins\" _" + gl(1) + "先查询再删除该目录中所有匹配文件和目录的旧版本。" + gl(2)
    + CMD + CMD_DEL_DIR_OLY_OLD_VER + "!* . \"E:/Java/eclipse/SpringToolSuite/plugins\" _" + gl(1) + "先查询再删除该目录中所有匹配目录的旧版本。" + gl(2)
