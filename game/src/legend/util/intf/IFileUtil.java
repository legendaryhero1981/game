...
public interface IFileUtil extends ICommon{
    ...
    + "limit" + gs(7) + "查询类命令（即命令选项以-f开头的命令）的查询结果显示数量限制，即显示前limit条记录；或同名文件替换类命令（见命令选项" + CMD_REP_FLE_SN + "）的最大查询层数，即只查询limit层目录结构；取值范围为：1~2147483647，不指定或指定一个非正整数则取默认值2147483647。" + gl(2)
    + "level" + gs(7) + "文件目录最大查询层数；取值范围为：1~2147483647，不指定或指定一个非正整数则取默认值2147483647。" + gl(2)
