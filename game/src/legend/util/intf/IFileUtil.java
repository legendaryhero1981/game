...
public interface IFileUtil extends IReplaceRule{
    ...
    + RULE_REPLACE + "(query" + SPRT_ARGS + "replacement[" + SPRT_ARGS + "mismatch])" + gs(23) + "将匹配query的列字符串的子串替换为replacement，将不匹配query的列字符串替换为mismatch，mismatch可以不指定；匹配的正则表达式为：" + REG_RULE_REPLACE + "；query为正则查询表达式，replacement为正则替换表达式且支持逻辑方法（形如：" + PH_ARGS + "逻辑方法表达式" + PH_ARGS + "，匹配的正则表达式为：" + REG_LOGIC_FUNC + "），mismatch表示不匹配query时则使用该字符串替换原始字符串；可使用特殊字符占位符表达式（见regex参数）。" + gl(2)
    ...
    + "目前原子规则" + RULE_REPLACE + "支持的所有逻辑方法（英文字母不区分大小写）如下：" + gl(2)
    + ADD_INT + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(8) + "group为query中指定的捕获组编号，取值范围为0~9；variable为整型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value += variable后再将value返回并替换旧值。" + gl(2)
    + ADD_LONG + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(7) + "group为query中指定的捕获组编号，取值范围为0~9；variable为长整型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value += variable后再将value返回并替换旧值。" + gl(2)
    + ADD_FLOAT + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(6) + "group为query中指定的捕获组编号，取值范围为0~9；variable为单精度浮点型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value += variable后再将value返回并替换旧值。" + gl(2)
    + ADD_DOUBLE + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(5) + "group为query中指定的捕获组编号，取值范围为0~9；variable为双精度浮点型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value += variable后再将value返回并替换旧值。" + gl(2)
    + SUB_INT + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(8) + "group为query中指定的捕获组编号，取值范围为0~9；variable为整型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value -= variable后再将value返回并替换旧值。" + gl(2)
    + SUB_LONG + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(7) + "group为query中指定的捕获组编号，取值范围为0~9；variable为长整型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value -= variable后再将value返回并替换旧值。" + gl(2)
    + SUB_FLOAT + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(6) + "group为query中指定的捕获组编号，取值范围为0~9；variable为单精度浮点型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value -= variable后再将value返回并替换旧值。" + gl(2)
    + SUB_DOUBLE + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(5) + "group为query中指定的捕获组编号，取值范围为0~9；variable为双精度浮点型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value -= variable后再将value返回并替换旧值。" + gl(2)
    + MUL_INT + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(8) + "group为query中指定的捕获组编号，取值范围为0~9；variable为整型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value *= variable后再将value返回并替换旧值。" + gl(2)
    + MUL_LONG + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(7) + "group为query中指定的捕获组编号，取值范围为0~9；variable为长整型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value *= variable后再将value返回并替换旧值。" + gl(2)
    + MUL_FLOAT + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(6) + "group为query中指定的捕获组编号，取值范围为0~9；variable为单精度浮点型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value *= variable后再将value返回并替换旧值。" + gl(2)
    + MUL_DOUBLE + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(5) + "group为query中指定的捕获组编号，取值范围为0~9；variable为双精度浮点型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value *= variable后再将value返回并替换旧值。" + gl(2)
    + DIV_INT + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(8) + "group为query中指定的捕获组编号，取值范围为0~9；variable为整型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value /= variable后再将value返回并替换旧值。" + gl(2)
    + DIV_LONG + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(7) + "group为query中指定的捕获组编号，取值范围为0~9；variable为长整型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value /= variable后再将value返回并替换旧值。" + gl(2)
    + DIV_FLOAT + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(6) + "group为query中指定的捕获组编号，取值范围为0~9；variable为单精度浮点型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value /= variable后再将value返回并替换旧值。" + gl(2)
    + DIV_DOUBLE + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(5) + "group为query中指定的捕获组编号，取值范围为0~9；variable为双精度浮点型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value /= variable后再将value返回并替换旧值。" + gl(2)
    + MOD_INT + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(8) + "group为query中指定的捕获组编号，取值范围为0~9；variable为整型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value %= variable后再将value返回并替换旧值。" + gl(2)
    + MOD_LONG + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(7) + "group为query中指定的捕获组编号，取值范围为0~9；variable为长整型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value %= variable后再将value返回并替换旧值。" + gl(2)
    + MOD_FLOAT + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(6) + "group为query中指定的捕获组编号，取值范围为0~9；variable为单精度浮点型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value %= variable后再将value返回并替换旧值。" + gl(2)
    + MOD_DOUBLE + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(5) + "group为query中指定的捕获组编号，取值范围为0~9；variable为双精度浮点型变量，设捕获组group对应的数值为value，则执行此方法等效于执行value %= variable后再将value返回并替换旧值。" + gl(2)
    + POW_INT + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(8) + "group为query中指定的捕获组编号，取值范围为0~9；variable为整型变量，设捕获组group对应的数值为value，则执行此方法等效于执行方法Math.pow(value,variable)后再将值返回并替换旧值。" + gl(2)
    + POW_LONG + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(7) + "group为query中指定的捕获组编号，取值范围为0~9；variable为长整型变量，设捕获组group对应的数值为value，则执行此方法等效于执行方法Math.pow(value,variable)后再将值返回并替换旧值。" + gl(2)
    + POW_FLOAT + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(6) + "group为query中指定的捕获组编号，取值范围为0~9；variable为单精度浮点型变量，设捕获组group对应的数值为value，则执行此方法等效于执行方法Math.pow(value,variable)后再将值返回并替换旧值。" + gl(2)
    + POW_DOUBLE + "(group" + SPRT_ARGS_FUNC + "variable)" + gs(5) + "group为query中指定的捕获组编号，取值范围为0~9；variable为双精度浮点型变量，设捕获组group对应的数值为value，则执行此方法等效于执行方法Math.pow(value,variable)后再将值返回并替换旧值。" + gl(2)
    ...
    + CMD + S_SPACE + CMD_REP_FLE_CS + OPTIONS + " regex src replacement [level]" + gl(1) + "根据regex将src中所有匹配文件的字符集编码转换为replacement编码；建议replacement的取值范围为（英文字母不区分大小写）：" + CHARSET_GBK + "，" + CHARSET_BIG5 + "，" + CHARSET_UTF8 + "（不带BOM），" + CHARSET_UTF8_BOM + "（带BOM），" + CHARSET_UTF16LE + "（带BOM），" + CHARSET_UTF16BE + "（带BOM）；原始文件字符集编码将被程序自动识别，目前不支持中文简繁编码之间的相互转换。" + gl(2)
    ...
    + CMD + S_SPACE + CMD_CMD + OPTIONS + " regex src [level]" + gl(1) + "根据regex查找src中的命令脚本文件，再逐一解析这些文件后按照每行一条命令的模式并发执行命令；此命令最适合的使用场景是并发执行多条耗时长的命令（譬如IO操作），若需要执行经过了数据筛选后得到的命令脚本文件，建议与" + CMD_REP_FLE_BT + "命令选项联合使用（即执行一条组合命令）。" + gl(2)
    + CMD + S_SPACE + CMD_7ZIP + OPTIONS + " regex src [level]" + gl(1) + "根据regex查找src中的配置文件，再逐一解析这些文件后并发调用7-Zip控制台程序执行压缩或解压缩命令。" + gl(2)
    + CMD + S_SPACE + CMD_DSRP + OPTIONS + " regex src [level]" + gl(1) + "根据regex查找src中的配置文件，再逐一解析这些文件后并发调用黑暗之魂系列游戏的专用控制台程序执行打包或解包命令。" + gl(2)
    ...
    + CMD + S_SPACE + CMD_REP_FLE_BT + "* (?i)\\A`temp1.txt`$ E:/Decompile/DLL-ildasm REPLACE(value=#DQM#([.]?\\d+|\\d+[.]\\d*)#DQM#,,value=#DQM##MUL-FLOAT(1,1.5)##DQM#) \\n 1" + gl(1)
    + "先查询再对该目录中名称（忽略大小写）为temp1.txt的文件数据执行一系列有序的替换规则：" + gl(1)
    + "1、对每行的所有列数据执行原子规则REPLACE：使用逻辑方法MUL-FLOAT将查询字符串中捕获组编号为1的所有value变量的值替换为原来的1.5倍；" + gl(1)
    + "例如：temp1.txt文件中有1行数据为：“<addBonus operator=\"ADD_PREMULT\" type=\"WEAPON_MAINHAND_DAMAGE\" value=\"15\"/><addBonus operator=\"ADD_PREMULT\" type=\"WEAPON_MAINHAND_INTERRUPT\" value=\"10\"/>”，则执行命令后该文件数据变为：“<addBonus operator=\"ADD_PREMULT\" type=\"WEAPON_MAINHAND_DAMAGE\" value=\"22.5\"/><addBonus operator=\"ADD_PREMULT\" type=\"WEAPON_MAINHAND_INTERRUPT\" value=\"15.0\"/>”。" + gl(2)
    ...
    + CMD + S_SPACE + CMD_REP_FLE_CS + "* (?i)`.txt`$ E:/Decompile/DLL-ildasm gbk" + gl(1) + "先查询再将“E:/Decompile/DLL-ildasm”目录中所有扩展名为.txt的文件的字符集编码转换为gbk编码。" + gl(2)
    ...
    + CMD + S_SPACE + CMD_CMD + "+ (?i)`curl.txt`$ \"F:/我的云盘/游戏/Wartales\" 1" + gl(1) + "先查询获得当前目录中（不包含子目录）文件名以curl.txt结尾（英文字母忽略大小写）的所有命令脚本文件，再逐一解析这些文件后按照每行一条命令的模式并发执行命令。" + gl(2)
    + CMD + S_SPACE + CMD_7ZIP + "+ (?i)`file-7zip.xml`$ . 1" + gl(1) + "先查询获得当前目录中（不包含子目录）文件名以file-7zip.xml结尾（英文字母忽略大小写）的所有配置文件，再逐一解析这些文件后并发调用7-Zip控制台程序执行压缩或解压命令。" + gl(2)
    + CMD + S_SPACE + CMD_DSRP + "+ (?i)`file-dsrp.xml`$ . 1" + gl(1) + "先查询获得当前目录中（不包含子目录）文件名以file-dsrp.xml结尾（英文字母忽略大小写）的所有配置文件，再逐一解析这些文件后并发调用黑暗之魂系列游戏的专用控制台程序执行打包或解包命令。" + gl(2)
