...
public interface IFileUtil extends ICommon{
    ...
    + "命令参数：" + gl(2)
    + "regex" + gs(7) + "文件名查询正则表达式，.匹配任意文件名和目录名。" + gl(2)
    ...
    + "replacement" + gs(1) + "字符串替换表达式，可作为文件名替换正则表达式；也可作为二维表格式文件中的列字符串替换表达式，格式为：[列号表达式" + SPRT_FID + "]规则1(参数列表)" + SPRT_RULE + "[规则2(参数列表) ... " + SPRT_RULE + "规则n(参数列表)]；若不指定列号表达式则对所有列执行指定的规则；规则具备事务性，简单事务性规则仅由1个原子规则组成，复杂事务性规则由多个原子规则组成；各规则通过" + SPRT_RULE + "分隔，复杂事务性规则中各原子规则通过" + SPRT_ATOM + "分隔，各参数通过" + SPRT_ARG + "分隔；" + gl(2)
    + "列号表达式格式形如：1（取第1列）；1,3,5（取1、3、5列）；1-3（取1、2、3列）；1,4-6（取1、4、5、6列）；" + gl(1)
    + "目前支持的所有原子规则名称（不区分大小写）如下：" + gl(1)
    + "LOWER(mstring)" + gs(11) + "将匹配mstring的列字符串中英文字母替换为小写；可以不传参数mstring，即LOWER与LOWER(.)等效但更高效；mstring为正则匹配表达式；" + gl(1)
    + "UPPER(mstring)" + gs(11) + "将匹配mstring的列字符串中英文字母替换为大写；可以不传参数mstring，即UPPER与UPPER(.)等效但更高效；mstring为正则匹配表达式；" + gl(1)
    + "REPLACE(mstring" + SPRT_ARG + "rstring)" + gs(1) + "将匹配mstring的列字符串的子串替换为rstring；mstring为正则匹配表达式，rstring为正则替换表达式；" + gl(1)
    + "REGENROW(tstring)" + gs(9) + "根据string重新生成每一行数据，此规则只能作为最后一条规则使用，即只能放在规则列表的最后面；tstring为行数据生成模版字符串，可使用列数据占位符表达式；" + gl(1)
    + "目前支持的所有列数据占位符表达式如下：" + gl(1)
    + "#n.m#" + gs(1) + "提取通过执行原子规则获得的列数据；n为列号，m为原子规则执行顺序号；m的最小值为0，最大值为原子规则执行总数；m取0表示提取n列的原始数据，当在列字符串替换表达式中指定了列号表达式且m取值大于0时，n的取值应在列号表达式中存在；例如：#1.0#（提取第1列的原始数据），#1.1#(提取对第1列执行了第1条规则后得到的数据)；" + gl(1)
    + "#n-m#" + gs(1) + "提取通过执行复杂事务性规则获得的列数据；n为列号，m为复杂规则执行顺序号；m的最小值为1，最大值为复杂规则执行总数；当在列字符串替换表达式中指定了列号表达式时，n的取值应在列号表达式中存在；例如：#1-1#（提取第1列执行了第1条复杂规则后得到的数据）。" + gl(2)
    + "命令选项：" + gl(2)
    ...
    + "@ 可添加在命令选项末尾，表示缓存该命令的查询结果，供后面的命令复用；某些命令不能缓存或复用查询结果，程序将智能忽略掉；复用查询结果的命令将忽略与查询相关的命令参数regex和src；当后面某个命令使用了@时，则重新缓存查询结果；可与+或*或?连用；例如：-f@*?。" + gl(2)
    + "? 可添加在命令选项末尾，表示命令开始执行前启用询问模式（" + ST_ASK_CONT + "）；可与+或*或@连用；例如：-f+@?。" + gl(2)
    + "组合命令：" + gl(2)
    ...
    + CMD + CMD_JSON_DEC + OPTIONS + "regex src [level]" + gl(1) + "根据regex查找src中的文件（同-f），解码（即格式化）JSON格式文件。" + gl(2)
    + "单条命令示例：" + gl(2)
    ...
    + CMD + CMD_REP_FILE + "+ (?i)\\Atemp1\\.txt$ E:/Decompile/DLL-ildasm 1##LOWER [ \t]+ 1" + gl(1) + "先查询（作用同-f）再将该目录中名称（忽略大小写）为temp1.txt的文件中每行的第1列数据中的英文字母全部替换为小写。" + gl(2)
    + CMD + CMD_REP_FILE + "* (?i)\\Atemp1\\.txt$ E:/Decompile/DLL-ildasm 1##UPPER=>REPLACE(\\.,,_);;REGENROW(String INST_#1-1# = #1.0#;) [ \t]+ 1" + gl(1) + "先查询（作用同-f）再对该目录中名称（忽略大小写）为temp1.txt的文件中每行的第1列数据进行一系列事务性操作：" + gl(1)
    + "1、将英文字母全部替换为大写；" + gl(1)
    + "2、；" + gl(1)
