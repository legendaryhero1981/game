...
public interface IFileUtil extends IReplaceRule{
    ...
    + "#EMPTY#" + gs(1) + "空字符串占位符表达式，匹配的正则表达式为：" + REG_SPC_EMPTY + "；表示一个空字符串，一般作为命令参数占位符（见-rfbt命令选项的replacement参数）；例如：#EMPTY#（程序将自动替换为空字符串）。" + gl(2)
    ...
    + RULE_DISTFINALROW + "(replacement[" + SPRT_ARGS + "join" + SPRT_ARGS + "prefix" + SPRT_ARGS + "suffix])" + gs(10) + "该规则的作用同规则" + RULE_GENFINALROW + "，区别在于它只会从根据replacement重新生成的每一行数据中取不重复的行数据。" + gl(2)
    ...
    + CMD + CMD_REP_FLE_BT + "* (?i)`native.log`$ d:/games  \"REPLACE(.*?--initialize-at-run-time=(.+?) .*,,$1,,#EMPTY#);;DISTFINALROW(#1.1#,,`,`,,--initialize-at-run-time=)\" \"\\n\" 1" + gl(1)
    + "先查询再对该目录中名称（忽略大小写）为native.log的文件数据执行一系列有序的替换规则：" + gl(1)
    + "1、对每行的第1列数据执行原子规则REPLACE：将所有匹配的列字符串替换为捕获组1，且将所有不匹配的列字符串替换为空字符串；" + gl(1)
    + "2、对每行数据执行原子规则DISTFINALROW：将数据替换为--initialize-at-run-time=concat(#1.1#,',')；" + gl(1)
    + "例如：native.log文件中有3行数据为：“Detailed message:" + gl(1)
    + "Error: Class initialization of com.sun.xml.bind.v2.ContextFactory failed. Use the option --initialize-at-run-time=com.sun.xml.bind.v2.ContextFactory to explicitly request delayed initialization of this class."+ gl(1)
