...
public interface IILCode extends ICommon{
    ...
    + gs(4) + "ILCodes::mode" + gs(11) + "对IL源文件内容的处理模式，取值范围为：" + MODE_NATIVE + "," + MODE_REPL + "，默认值为" + MODE_NATIVE + "；取" + MODE_NATIVE + "表示先根据ILCodes::ILCode查询定位IL源文件中需要修改的内容，再自动生成已修改的IL源文件；取" + MODE_REPL + "表示根据ILCodes::ILCode直接自动生成已修改的IL源文件。" + gl(1)
    + gs(4) + "ILCodes::ILCode" + gs(9) + "ILCode配置节点，包括对IL代码的处理模式和修改IL代码片段所需的相关参数等等。" + gl(1)
    + gs(4) + "ILCode节点由processingMode、lineNumber、codeDesc、queryRegex、codeRegex、codeFragment节点按顺序组成；processingMode节点必须在最前面。" + gl(1)
    + gs(4) + "ILCode::processingMode" + gs(2) + "在自动生成已修改的IL源文件时对IL代码的处理模式，取值范围为：" + MODE_NATIVE + "," + MODE_REPL + "," + MODE_ADD + "，默认值为" + MODE_NATIVE + "；取" + MODE_NATIVE + "表示提取IL源文件的原始数据；取" + MODE_REPL + "表示提取ILCode::codeFragment作为替换数据；取" + MODE_ADD + "表示提取ILCode::codeFragment作为新增数据。" + gl(1)
    ...
    + gs(4) + "ILCode::queryRegex" + gs(6) + "在IL源文件中查询定位要修改的IL代码片段时所需的正则查询表达式；仅当ILCode::processingMode不为" + MODE_NATIVE + "时有效；若ILCode::processingMode为" + MODE_REPL + "，将继续向上回溯查询ILCode::codeRegex以便于精确定位。" + gl(1)
    + gs(4) + "ILCode::codeRegex" + gs(6) + "在IL源文件中查询定位要修改的IL代码片段时所需的正则查询表达式；仅当ILCode::processingMode为" + MODE_REPL + "时有效。" + gl(1)
