...
~ 可添加在命令选项末尾，表示显示最简明的信息，也不会显示进度条；优先级高于+；若未指定则默认显示进度条；可与其它后命令选项连用；例如：-fd~!@?。

+ 可添加在命令选项末尾，表示输出详细信息；优先级低于~和*；可与其它后命令选项连用；例如：-fd!+@?。

* 可添加在命令选项末尾，表示模拟执行命令，不进行实际操作，仅输出详细信息；优先级高于+和~，且与~联用时只输出最简明的信息；可与其它后命令选项连用；例如：-fd*?@!。

! 可添加在命令选项末尾，表示不匹配查询的根目录，可与其它后命令选项连用；例如：file -fd!+ . d:/games 不匹配games目录，只匹配该目录中的任意文件和子目录名称。

^ 可添加在命令选项末尾，表示对描述目录或文件名的正则表达式匹配结果做取反操作，一般用于需要取反一个或多个字符串的情况，此功能弥补了零宽断言描述字符串取反的局限性；可与其它后命令选项连用；例如：-fd^!+@?。

@ 可添加在命令选项末尾，表示缓存该命令的查询结果，供后面的命令复用；某些命令不能缓存或复用查询结果，程序将智能忽略掉；复用查询结果的命令将忽略与查询相关的命令参数regex和src；当后面某个命令使用了@时，则重新缓存查询结果；可与其它后命令选项连用；例如：-fd!@*?。

? 可添加在命令选项末尾，表示命令开始执行前启用询问模式（输入n或N跳过，否则继续，按回车键确认：）；可与其它后命令选项连用；例如：-fd!+@?。
